...
export default function ChoreVacationSettings({ choreId }) {
  ...
  const fetchVacations = useCallback(async () => {
    try {
      const data = await api(`/api/chores/${choreId}/vacations`);
      setVacations(data);
    } catch {
      // ignore
    } finally {
  ...
  const create = async () => {
    ...
    setError('');
    try {
      await api(`/api/chores/${choreId}/vacations`, {
        method: 'POST',
  ...
  const cancel = async (id) => {
    try {
      await api(`/api/chores/${choreId}/vacations/${id}`, { method: 'DELETE' });
      fetchVacations();
    } catch {
      // ignore
    }
  ...
  return (
    <div className="game-panel p-5 space-y-3">
      ...
      <p className="text-muted text-xs">
        {t('choreVacation.description')}
      </p>

      {showForm && (
        <div className="p-3 rounded-lg bg-surface-raised/50 border border-border/50 space-y-3">
