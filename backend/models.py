...
class Reward(Base):
    ...
    # When true, the reward is redeemed automatically for any kid the moment
    # their star balance reaches point_cost.
    auto_purchase: Mapped[bool] = mapped_column(Boolean, default=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_by: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class FamilyTodo(Base):
    """A simple shared to-do item on the Family Zone screen (shopping list,
    reminders, etc.) -- unrelated to chores or points."""
